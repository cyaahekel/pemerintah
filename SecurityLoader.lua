-- UPDATED SECURITY LOADER - Includes EventTeleportDynamiefws4rw4r
...
-- ============================================
-- ENCRYPTED MODULE URLS (CONSOLIDATED)
-- ============================================
local encryptedURLs = {
...
local function loadAllModules()
    if loadedModules then return loadedModules end
    
    local encrypted = encryptedURLs["CombinedModules"]
    if not encrypted then
        warn("❌ CombinedModules URL not found")
        return nil
    end
    ...
    if not success then
        warn("❌ Failed to load CombinedModules:", result)
        return nil
    end
...
function SecurityLoader.LoadModule(moduleName)
    ...
    local allModules = loadAllModules()
    if not allModules then
        return nil
    end
    
    local module = allModules[moduleName]
    if not module then
